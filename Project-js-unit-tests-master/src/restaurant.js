...
const createMenu = (newMenu) => {
  ...
  return {
    ...
    pay: () => {
      ...
      for (const consumedItem of consumption) {
        const foodItemValue = food[consumedItem]
        const drinkItemValue = drinks[consumedItem]
        if (foodItemValue) {
          total += foodItemValue
        }
        if (drinkItemValue) {
          total += drinkItemValue
        }
      }
      return Number((total * 100).toFixed(2) / 100)
    }
  }
}
